...
pub fn test(clang: &Clang) {
    ...
    super::with_file(&clang, "int a = 322;", |p, f| {
        ...
        assert!(f.get_time() != 0);
        super::with_file(&clang, "int a = 322;", |_, g| {
            assert!(f.get_id() != g.get_id())
        });
    ...
    let files = &[
        (
            "module.modulemap",
            "module parent { module child [system] { header \"test.hpp\" } }",
        ),
        ("test.hpp", ""),
    ...
    super::with_temporary_files(files, |_, fs| {
        ...
        if cfg!(feature = "clang_3_6") {
            ...
            assert_eq!(
                module.get_file().get_path().extension(),
                Some(OsStr::new("pcm"))
            );
            assert_eq!(module.get_full_name(), "parent.child");
            assert_eq!(module.get_name(), "child");
            assert_eq!(
                module.get_top_level_headers(),
                &[tu.get_file(&fs[1]).unwrap()]
            );
    ...
    super::with_file(&clang, source, |_, f| {
        ...
        assert_eq!(
            location.get_presumed_location(),
            ("presumed.hpp".into(), 321, 33)
        );
